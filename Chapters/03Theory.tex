...
\begin{equation}
\theta(t) = \omega \left(GMST(t) - (UT1(t) - UTC(t))\right), \qquad
\omega = 0.261799387799149~\text{rad/h}.
\end{equation}
...
\begin{equation}
    \textbf{a}_{grav} = \ddot{\textbf{r}}_{s/i} = - \frac{\mu}{||\textbf{r}_{s/i}||_2^3} \textbf{r}_{s/i} \iff \ddot{\textbf{r}}_{s/i} = \nabla U, \qquad \text{where } U = \mu\frac{1}{||\textbf{r}_{s/i}||_2}
...
\begin{equation}
    \textbf{a}_{grav}^f = \ddot{\textbf{r}}^f_{s/i} = \nabla U(r, \phi^f) \;\;\Longleftrightarrow\;\; \textbf{a}_{grav}^i = \ddot{\textbf{r}}^i_{s/i} = \textbf{R}^i_f \nabla U(r, \phi^f)
\end{equation}
% Feedback from chat:
